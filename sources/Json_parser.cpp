...
std::ostream& operator<<(std::ostream& out, Json_parser& J) {
  size_t size = 0;
  ...
  J.length_of_fields[0] = size + 3;

  out << std::left;
  for (size_t i = 0; i < 4; ++i) {
    out << std::setw(J.length_of_fields[i]) << "| " + J.columns[i];
  }
  out << "|" << std::endl;
  for (size_t i = 0; i < 4; ++i) {
    out << std::setfill('-') << std::setw(J.length_of_fields[i]) << '|';
  }
  size_t Size = 0;
  for (auto i : J.students) {
    if (i.getName().length() > Size) {
    ...
    if ((std::any_cast<json>(J.students[i].getDebt()).is_array())) {
      if ((std::any_cast<json>(J.students[i].getDebt()).size()) > 1) {
        out << "| " << std::any_cast<json>(J.students[i].getDebt()).size()
            << std::setw(J.length_of_fields[3] - 3) << " items";
      } else {
        std::vector<std::string> d =
            std::any_cast<json>(J.students[i].getDebt())
                .get<std::vector<std::string>>();
        out << "| " << std::setw(J.length_of_fields[3] - 2) << d[0];
      }
    } else if ((std::any_cast<json>(J.students[i].getDebt()).is_string())) {
      out << "| " << std::setw(J.length_of_fields[3] - 2)
          << std::any_cast<json>(J.students[i].getDebt()).get<std::string>();
    } else {
      out << std::setw(J.length_of_fields[3]) << "| null";
    }
    out << "|";
    out << std::endl;
    for (size_t t = 0; t < 4; ++t) {
      out << std::setfill('-') << std::setw(J.length_of_fields[t]) << "|";
    }
    out << "|" << std::endl;
    out << std::setfill(' ');
  }
